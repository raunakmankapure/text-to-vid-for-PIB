...
interpreter = interpreter.load("models\\20240325-115525-counting-margin.tar.gz")
interpreter = Interpreter.load("models\\20240325-115525-counting-margin.tar.gz")  # Replace with your model path
# Replace with your model path

@approval.route('/chat', methods=['POST'])
def chat():
    if request.method == 'POST':
        message = request.form['message']

        # Use Rasa interpreter to process the message
        data = interpreter.parse(message)
        intent = data["intent"]["name"]  # Get the predicted intent
        entities = data["entities"]  # Get any extracted entities

        response = ""
        if intent == "greet":
            response = "Hello! How can I help you?"
        elif intent == "ask_question":
            # Handle questions based on extracted entities or keywords in the message
            question_type = ...  # Determine the type of question based on analysis
            response = answer_question(question_type)  # Replace with your function to answer questions
        else:
...
def search():
    if request.method == 'POST':
        ...
        speech_url = "https://api.elevenlabs.io/v1/text-to-speech/8Ee8aTGcSYfUOvTaKtWS"
        headers = {
            "Accept": "audio/mpeg",
            "Content-Type": "application/json",
            "xi-api-key": "38e80055ba14a784d6eab943f5070f56"
        }
        speech_data = {
